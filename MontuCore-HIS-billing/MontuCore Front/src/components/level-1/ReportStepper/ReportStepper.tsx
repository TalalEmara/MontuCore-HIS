...
export default function ReportStepper({
  ...
  const generatePDF = (data: Partial<FullReport>, save = false) => {
    ...
    if (data.symptoms?.length) data.symptoms.forEach((s,i) => addLine(`Symptom ${i+1}`, s));
    ...
    doc.text("This report is generated by MontuCore Medical System. Confidential and intended for medical use only.", 105, y, { align: "center" });
        ...
        <div className={styles.mainBody}>
          <div className={styles.stepper}>
            {[1,2,3].map((step)=>(
                ...
                <div className={styles.stepLabel}>{step===1?"Patient":step===2?"Symptoms":step===3?"Assessment":"Treatment"}</div>
              </div>
            ))}
          </div>

          <form className={styles.content} onSubmit={form.handleSubmit(handleFinalSubmit)}>
            {currentStep===1 && <Step1Symptoms form={form} />}
            {currentStep===2 && <Step2Assessment form={form} />}
            {currentStep===3 && <Step3Treatment form={form} />}

            <div className={styles.actions}>
              <Button type="button" variant="secondary" onClick={handleBack} disabled={currentStep===1}>Back</Button>
              <div style={{display:"flex", gap:"6px"}}>
                <Button type="button" variant="secondary" onClick={handleViewReport}>View Report</Button>
                {currentStep < 3 
