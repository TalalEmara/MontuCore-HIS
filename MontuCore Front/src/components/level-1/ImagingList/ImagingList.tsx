import React from 'react';
import styles from './ImagingList.module.css';
import List from '../../level-0/List/List';
import Badge from '../../level-0/Badge/badge';
import { useNavigate } from '@tanstack/react-router';

type ImagingListProps = {
  data :string[][];
  Route: any ;
};

function checkDataStructure(data: String[][], headerLength: number): boolean {
  return data.every(row => row.length === headerLength);
}

function ImagingList({data , Route}: ImagingListProps) {
  // chekk data structure number of columns match header length
  // extract the certain data
  // the avatar path handeling and not found case
  // status rendering with colors
  // button no added no data (should be passed as cell)
  const navigate = useNavigate();
  const headerLabels = ['id', 'name', 'status', 'performed at'];
  if (!checkDataStructure(data, headerLabels.length+1)) {
    throw new Error("Data structure does not match header length");
  }
  const processedData = data.map(row => [
    row[0], // id
    <span className={styles.name}>{row[2]}:{row[1]}</span>, // name
    // need to be  a badge
    row[3] === "IMAGING_COMPLETE"? <Badge label= "completed" variant = 'success'/> : <Badge variant='pending' label={''}/>,
   
    row[4]
  ]);
  
  const handleScanClick = () => {
    navigate({
      to:Route
    });
  };
  // const data = data
  return (
    <List data={processedData} header={headerLabels} onClick={handleScanClick}/>
  );
}
export default ImagingList;